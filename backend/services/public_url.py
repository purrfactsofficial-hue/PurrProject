...
@contextmanager
def public_url_context(
    video_path: Path, episode_id: int, settings, language: str = "en"
...
@contextmanager
def _tunnel_context(video_path: Path, episode_id: int, language: str) -> Generator[str, None, None]:
    ...
    try:
        ...
        yield f"{tunnel_url}/media/{episode_id}/{language}.mp4"
    finally:
        proc.terminate()
        proc.wait()
